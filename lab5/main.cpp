...
void addEmployee(stringstream& lineStream) {
  ...
  bool exists = IDTree->searchID(ID);
  if (!exists) {
...
void searchAgeRangeEmployee(stringstream& lineStream) {
  ...
  ageTree->searchAgeRange(lowAge, highAge);  // this function does it all!
}
...
void autocompleteEmployee(stringstream& ss) {
  ...
  string prefix;
  if (!getString(ss, prefix)) {  // prefix empty or only contains whitespace
    return;
  }
